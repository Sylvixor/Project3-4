...
| 7  | Als eindgebruiker wil ik mijn saldo kunnen zien tijdens de interactie, zodat ik weet hoeveel geld ik nog heb voor opnames. | Saldo-overzicht beschikbaar. | Saldo wordt weergegeven op de GUI. | Gebruiker ziet saldo voordat een opname wordt gedaan. |
| 8  | Als bank wil ik dat een transactie stopt als de eindgebruiker wegloopt zonder af te ronden, zodat onbevoegden geen toegang krijgen tot het geld. | Automatische annulering. | Tijdslimiet op transacties implementeren. | Transactie wordt geannuleerd als gebruiker niet op tijd reageert. |
...
| 10 | Als bank wil ik dat de pinautomaat een oneindig aantal gebruikers aankan, zodat het systeem schaalbaar blijft voor nieuwe eindgebruikers. | Onbeperkte gebruikerscapaciteit. | De database heeft geen limiet voor het aantal gebruikers die kunnen worden aangemaakt | Geen limiet op aantal transacties of gebruikers. |
| 11 | Als bank wil ik eindgebruikersgegevens kunnen beheren (maar niet het saldo), zodat ik accounts kan aanmaken en updaten zonder financiële toegang. | Gebruikersbeheer zonder saldo-toegang. | Behering van de bank via de database | Bank kan accounts aanmaken/wijzigen zonder saldo-inzage. |
| 12 | Als eindgebruiker wil ik na drie foutieve pincode-invoeringen een waarschuwing krijgen, zodat ik weet dat mijn pas geblokkeerd wordt bij de volgende fout. | Pincode beveiliging. | Waarschuwingsmelding na 3 foute pogingen. | Gebruiker wordt gewaarschuwd na 3 fouten. |
| 13 | Als bank wil ik passen kunnen blokkeren en deblokkeren, zodat verloren of gestolen passen beveiligd kunnen worden. | Pasbeheerfunctie. | Functie om passen te blokkeren/deblokkeren. | Blokkade en deblokkade functionaliteit beschikbaar. |
| 14 | Als eindgebruiker wil ik mijn contactloze pas kunnen gebruiken bij de pinautomaat, zodat ik sneller en gemakkelijker geld kan opnemen. | Contactloos pinnen. | RFID sensor coderen, RFID implementeren in de pinautomaat, contactloze bankpas connecten aan een gebruiker | Pinautomaat accepteert contactloze kaarten. |
...
| 17 | Als eindgebruiker wil ik dat de pinautomaat automatisch teruggaat naar het beginscherm bij fouten, zodat de machine niet vastloopt en ik een nieuwe poging kan doen. | Automatische reset bij fouten. | Systeem keert terug naar startscherm bij een system error. | Automaat blijft bruikbaar na een fout. |
| 18 | Als bank wil ik dat de eindgebruiker minimaal 20 euro per transactie kan opnemen, zodat er minder transacties plaatsvinden en dus de automaat minder vaak bijgevuld hoeft te worden. | Minimumbedrag per opname. | Opnames vanaf minimaal 20 euro toegestaan. | Gebruiker kan geen bedragen onder 20 euro opnemen. |
...
| 20 | Als bank wil ik dat de pinautomaat kan communiceren met de NOOB-server, zodat transacties correct verwerkt worden. | Verbinding met NOOB-server. | NOOB server onderzoeken, NOOB server connecten aan de database, NOOB server communicatie met andere banken. | Transacties worden correct verwerkt via NOOB-server. |
| 21 | Als bank wil ik dat de database van de pinautomaat eindgebruiker gegevens zoals naam, e-mail, IBAN en saldo bevat, zodat eindgebruiker beheer en transacties goed werken. | Pinautomaat-database. | Keuze database onderzoeken, database laten werken met de NOOB server, tabellen en kolommen toevoegen in de database. | Gegevens correct opgeslagen en beheerd. |
